...
capillary_EC <- readRDS("/5_Endothelial_cell/3_sub_capillary_EC/capillary_EC.rds")

capillary_EC@meta.data$cell.type <- paste0("Capillary", capillary_EC@meta.data$RNA_snn_res.0.8)
Melanocyte@meta.data$cell.type <- paste0("Mel_", Melanocyte@meta.data$RNA_snn_res.0.3)

### merge
seurat_object_list <- list(capillary_EC=capillary_EC, Melanocyte=Melanocyte)
...
out_dir = "/5_Endothelial_cell/5_cellchat/"
...
# save data
saveRDS(cellchat, file= "/5_Endothelial_cell/5_cellchat/Mel_CapEC_cellchat.rds")

# plot
pathways.show =  pathways.show  # "VEGF" "MK", "SEMA3", "SPP1"
pdf("/5_Endothelial_cell/capillary_EC/CCI/hierarchy.pdf")
netVisual_aggregate(cellchat, signaling = pathways.show, vertex.receiver = c(1,2,3,4,5), layout = "hierarchy", color.use = c("#4cbbd2", "#e9949e", "#9fc6db", "#efb4cc", "#c4aca6", "#db6657", "#977dac", "#b1ccea", "#4876b4"))
dev.off()
